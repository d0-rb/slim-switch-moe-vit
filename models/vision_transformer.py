...
from .layers import MultipleSequential
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # if self.training:
        self.x_cls_attn = attn[:, :, 0, :]  # [B x H x N x N]
...
class SwitchableVisionTransformer(nn.Module):
        ...
        self.mid_blocks = nn.Sequential(
            *[
                Block(
                    ...
                    drop=drop_rate,
                    attn_drop=attn_drop_rate,
                    drop_path=dpr[i + (self.router_start % depth)],
                    norm_layer=norm_layer,
                    act_layer=act_layer,
                    num_tokens=num_patches + self.num_tokens,
                )
        ...
        self.post_blocks = nn.Sequential(
            *[
                Block(
                    ...
                    drop=drop_rate,
                    attn_drop=attn_drop_rate,
                    drop_path=dpr[i + (self.router_end % depth)],
                    norm_layer=norm_layer,
                    act_layer=act_layer,
                    num_tokens=num_patches + self.num_tokens,
                )
    ...
    def forward_features(
        self, x, bucket: Tensor | int | None = None, threshold: int = 0
    ):
        ...
        if self.routing:
            ...
            index_x = (
                torch.arange(self.router.switchable_buckets)
                .unsqueeze(0)
                .broadcast_to(passthru_mask.shape)
            )
