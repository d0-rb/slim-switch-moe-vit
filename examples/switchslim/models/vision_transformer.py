...
class MoEBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, num_tokens=-1,
                 num_expert=32, expert_dp_comm="none", expert_rank=0):
        super().__init__()
        self.norm1 = norm_layer(dim)  # layernorm (token norm)
        # self.norm1 = norm_layer(num_tokens)  # batchnorm1d (seq norm)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        ...
        # self.norm2 = norm_layer(num_tokens)  # batchnorm1d (seq norm)
        mlp_hidden_dim = int(dim * mlp_ratio)
        # self.mlp = FMoETransformerMLP(num_expert=num_expert, d_model=dim, d_hidden=mlp_hidden_dim,
        #                               activation=act_layer, expert_dp_comm=expert_dp_comm,
        #                               expert_rank=expert_rank)  # ignore drop for now :(
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class SwitchableVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', buckets=1, collect_embeddings=False):
        """
        Args:
...
class MoEVisionTransformer(nn.Module):
    ...
    Includes FMoE MLP layer
    """

    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, representation_size=None, distilled=False,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, weight_init='', num_expert=32, **kwargs):
        """
        Args:
            img_size (int, tuple): input image size
            patch_size (int, tuple): patch size
        ...
        self.blocks = nn.Sequential(*[
            MoEBlock(
                ...
                num_tokens=num_patches + self.num_tokens, num_expert=num_expert)
            for i in range(depth)])
        self.norm = norm_layer(embed_dim)  # layernorm (token norm)
        # self.norm = norm_layer(num_patches + self.num_tokens)  # batchnorm1d (seq norm)
