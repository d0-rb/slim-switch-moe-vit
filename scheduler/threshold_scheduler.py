...
__all__ = ['ConstantLR', 'LinearLR', 'LinearLRWarmup', 'CosineAnnealingLR', 'CosineAnnealingLRWarmup', 'LRScheduler']
...
class LinearLRWarmup(LRScheduler):
    ...
    def __init__(self, optimizer, warmup_steps=0, start_factor=1.0 / 3, end_factor=1.0, total_iters=5, last_epoch=-1,
                 verbose=False):
        # if start_factor > 1.0 or start_factor <= 0:
        #     raise ValueError('Starting multiplicative factor expected to be greater than 0 and less or equal to 1.')
...
class CosineAnnealingLRWarmup(LRScheduler):
    ...
    def __init__(self, optimizer, T_max, warmup_steps=0, eta_min=0, last_epoch=-1, verbose=False):
    ...
    def get_lr(self):
        ...
        if self.last_epoch == 0:
            ...
            return [group['lr'] for group in self.optimizer.param_groups]
        elif self._step_count == 1 and (self.last_epoch - self.warmup_steps) > 0:
            return [self.eta_min + (base_lr - self.eta_min) *
                    (1 + math.cos((self.last_epoch - self.warmup_steps) * math.pi / self.T_max)) / 2
                    for base_lr, group in
                    zip(self.base_lrs, self.optimizer.param_groups)]
        elif (self.last_epoch - self.warmup_steps - 1 - self.T_max) % (2 * self.T_max) == 0:
            return [group['lr'] + (base_lr - self.eta_min) *
                    (1 - math.cos(math.pi / self.T_max)) / 2
                    for base_lr, group in
                    zip(self.base_lrs, self.optimizer.param_groups)]
        return [(1 + math.cos(math.pi * (self.last_epoch - self.warmup_steps) / self.T_max)) /
