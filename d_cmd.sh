...
CUDA_VISIBLE_DEVICES=$cuda python3 -m torch.distributed.launch --nproc_per_node=$num_cuda --master_port=$RANDOM --use_env main.py --model $model --data-set CIFAR10 --data-path ./dataset --batch 128\
                ...
                --starting-threshold $starting_threshold --target-threshold $target_threshold \
                --output_dir cifar-models/$model/scratch_${starting_threshold}_${target_threshold}_${num_tasks}_rehearsal/$n \
                --num-tasks $num_tasks \
