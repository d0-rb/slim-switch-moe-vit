...
def forward_residule_moe_convit(self, x, mask_heads=None, task_index=1, attn_mask=None):
    if isinstance(self.attn, ClassAttention) or isinstance(self.attn, JointCA):  # Like in CaiT
        cls_token = x[:, :task_index]
    ...
    if self.is_cls_tk:
        cls_token = xx[:, 0].unsqueeze(dim=1)
        patch_idx += 1
    if self.is_dist_tk:
        dist_token = xx[:, 1].unsqueeze(dim=1)
        patch_idx += 1

    patch_tk = xx[:, patch_idx::]

    tokens, skip_tk, summary_token, summary_skip_token = self.dense_gate(patch_tk)  # , 1::])
    ...
    tokens_fwd, attn, v = self.attn(tokens)
    tokens_fwd = self.drop_path(tokens_fwd)
    if summary_token:
    ...
    x = mask_and_forward(
        x,
        self.moe_gate,
        lambda x: self.drop_path(self.mlp(x)) + x,
        self.is_cls_token,
        self.is_dist_token,
    )
...
def resmoe_tiny_patch16_224_expert8_convit(
    ...
    model = ConVit(pretrained=pretrained, **kwargs)
    ...
    for name, module in model.named_modules():
        if isinstance(module, Block):
            ...
            bound_method = forward_residule_moe_convit.__get__(
